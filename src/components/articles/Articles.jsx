    ...
    <>
      <form className="relative block w-full search-articles" onSubmit={(event) => { event.preventDefault(); onSearch(event.target.search.value, false); }}>
        <input
          ...
          {search &&
            <button aria-label="Clear search" className="absolute block top-0 right-0 text-green-700 w-6 h-6 box-content px-3 py-2" onClick={(event) => event.currentTarget.parentElement.search.value = ''}>
